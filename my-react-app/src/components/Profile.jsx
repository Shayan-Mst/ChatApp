...
import { getProfileApi, updatePictureApi } from '../api/users';
...
birthday:"",
    ...
    //triggering input click on svg button click
      ...
      try{
        ...
        if(response.status == 200){
          ...
          if(response.data.profile.profilePicture){
            setImage(response.data.profile.profilePicture)
            setIsEmpty(false)
          }
          setLoading(false)
        }
...
console.log(error)
    ...
    const changePicture = async(e) => {
      ...
      try{
        ...
        const response = await updatePictureApi(image)

        if(response.status==200){
          setCopySuccess("Profile Picture Changed Successfuly")
...
<div>
    ...
    <div className='my-4'>
    <button onClick={changePicture}  className='btn-one max-w-md '>
